...
| File | Purpose |
|---|---|
| `main.py` | Runs everything: camera loop, gesture dispatch, face/emotion, UI |
| `config.py` | All tunable constants |
| `settings.py` | Loads `settings.json` / `calibration.json` overrides at startup |
| `settings_gui.py` | Tkinter form to edit settings without touching `config.py` |
...
| `tray.py` | System tray icon with a Quit option |
| `ui.py` | Dark, semi-transparent HUD overlay + help screen |
...
## Real Trained Emotion Model (optional upgrade)
...
A sidebar-navigated desktop app (CustomTkinter) wraps everything above:
Dashboard (live camera + stats + quick actions), Settings (edit thresholds
without touching `config.py`), Logs (browse session CSVs), About (gesture
cheat-sheet).
...
`frontend/assistant_worker.py` runs the camera/gesture/face loop on a
background thread and feeds frames + stats to the GUI instead of opening
its own OpenCV window (that one file wasn't on your original list, but the
Dashboard needs a frame source to display).
...
├── sidebar.py            # nav rail
├── camera_frame.py       # live camera feed widget
├── dashboard.py          # camera + live stats + quick action buttons
├── settings_page.py      # editable config form -> settings.json
├── logs_page.py          # browse logs/*.csv
├── about_page.py         # gesture cheat-sheet
├── assistant_worker.py   # background thread bridging to the backend modules
├── assets/
│   ├── logo.png
│   └── icons/            # dashboard.png, settings.png, logs.png, about.png
└── widgets/
    ├── nav_button.py
    └── stat_card.py
```
...
- Session logs land in `logs/`, screenshots in `screenshots/`.
